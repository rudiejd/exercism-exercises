defmodule NewPassport do
  @spec get_new_passport(NaiveDateTime.t(), Date.t(), any()) :: any()
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
    {:ok, counter_number} <- find_counter_information(now),
    {:ok, checksum} <- stamp_form(timestamp, counter_number.(birthday), form),
    passport_number <- get_new_passport_number(timestamp, counter_number.(birthday), checksum)
    do
      {:ok, passport_number}
      else
